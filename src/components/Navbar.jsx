...
const Navbar = () => {
    ...
    const { showSearch, setShowSearch, getCartCount, navigate, token, setToken, setCartItems } =
    ...
    return (
        <div className="flex items-center justify-between py-5 font-medium">
            <Link to={'/'}>
                <img src={Logo} className="w-28" alt="logoNowhere_sg" />
            </Link>
            ...
            <div className="flex items-center gap-6">
                <BiSearch onClick={() => setShowSearch(!showSearch)} size={24} className="w-5 cursor-pointer" />
            ...
            <div
                className={`cursor-pointer absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'} md:hidden`}
            >
                <div onClick={() => setVisible(false)} className="flex flex-col text-gray-600">
                    <div className="flex items-center gap-4 p-3">
                        <IoIosArrowForward className="h-4 rotate-180 cursor-pointer" />
                        <p>Back</p>
                    </div>
